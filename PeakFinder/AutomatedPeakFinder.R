# LICOR CO2 peak extraction pipeline - generalized maxima-first version v4
# -------------------------------------------------------------------------
# Goal:
...
#
# Main v4 change:
#   Keeps the detrended maxima-first detector, then adds two general pruning rules:
#     1) Require stronger minimum prominence to remove small shoulders/noise.
#     2) Require a two-sided peak shape, meaning a candidate must have a real
#        valley on both sides. One-sided boundary candidates are usually shoulders
#        on the tail of a larger injection peak, not independent samples.
#   The expected count is used only as a validation warning, never as a target.
...
message("Detected final peaks: ", n_peaks)
